...
@push('styles')
    <link rel="stylesheet" href="{{ asset('css/rooms/edit.css') }}">
@endpush
...
<div class="edit-room-wrapper">
    <div class="page-header">
        <h2><i class="fa-solid fa-pen-to-square" style="color: var(--color-warning); margin-right: 8px;"></i>Edit Room</h2>
        <a href="{{ route('rooms.index') }}" class="btn btn-secondary btn-sm">
            <i class="fa-solid fa-arrow-left"></i> Back to Rooms
        </a>
    </div>

    <form action="{{ route('rooms.update', $room->id) }}" method="POST" class="room-edit-form">
        @csrf
        @method('PUT')
        ...
        <div class="form-section">
            ...
            <div class="current-info">
                ...
                <span>Editing Room <strong>{{ $room->room_number }}</strong></span>
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <select name="bed_type" id="bed_type" class="form-select" required>
                        ...
                        <option value="double" {{ old('bed_type', $room->bed_type) == 'double' ? 'selected' : '' }}>🛏️🛏️ Double</option>
                    </select>
                    @error('bed_type')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <select name="has_wifi" id="has_wifi" class="form-select">
                        <option value="1" {{ old('has_wifi', $room->has_wifi) == '1' ? 'selected' : '' }}>📶 Yes</option>
                        <option value="0" {{ old('has_wifi', $room->has_wifi) == '0' ? 'selected' : '' }}>🚫 No</option>
                    </select>
                    @error('has_wifi')
                        <p class="form-error">{{ $message }}</p>
                    @enderror
        ...
        <div class="form-actions">
            ...
            <button type="submit" class="btn btn-primary btn-lg">
                <i class="fa-solid fa-check"></i> Update Room
            </button>
        </div>
    </form>
</div>
